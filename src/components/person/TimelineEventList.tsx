...
import type { PersonEvent, EventType } from '@/lib/types';
...
interface Props {
  events: PersonEvent[];
  onEdit: (event: PersonEvent) => void;
  onDelete: (event: PersonEvent) => void;
}

export function TimelineEventList({ events, onEdit, onDelete }: Props) {
  if (events.length === 0) {
    return (
    ...
    <div className="relative">
      ...
      <div className="space-y-6">
        {events.map((event) => {
          ...
          const end = event.endDate ? timestampToDate(event.endDate) : null;
          return (
            <div key={event.id} className="relative flex gap-4 pl-10">
              <div className="absolute left-0 flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400">
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={EVENT_ICONS[event.type]} />
                </svg>
              </div>
              <div className="flex-1 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    <button type="button" aria-label={`Edit ${event.title}`} onClick={() => onEdit(event)}
                    ...
                    <button type="button" aria-label={`Delete ${event.title}`} onClick={() => onDelete(event)}
